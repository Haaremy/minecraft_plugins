...
| **OpenAudioMc** | Bukkit, optional | Required only for hmyLobby stream jukeboxes |
...
Each plugin is a Maven project. Build a single plugin from its directory:
...
Deploy the generated JAR from `target/` to the matching Velocity or Paper `plugins/` directory, then restart only the affected server.

### Runtime Paths

Production currently uses this layout:

```text
/root/minecraft/
  velocity.jar
  plugins/                  # Velocity plugins and data
  lobby/plugins/            # Lobby Paper plugins
  vanilla/plugins/          # Survival Paper plugins
  kitsune/plugins/          # Kitsune/game Paper plugins
  hmySettings/              # Shared settings for selected plugins
  hmyLanguages/             # Shared language files
```

Some plugins use their own Bukkit data folder under `plugins/<PluginName>/`; others use shared `hmySettings/`. The plugin-specific README is authoritative for the setup file.

> **Player guides:** for in-game how-to documentation (gameplay, commands players need, rewards) see [GAMES.md](GAMES.md) and the `PLAY.md` next to each game-mode plugin.
...
Daily reward plugin with a claim GUI, streak tracking, hmyCore economy integration, optional LuckPerms temporary ranks, and a 30-day reward overview.

### Commands

| Command | Description | Permission |
|---------|-------------|------------|
| `/daily` | Open the daily reward GUI | — |
| `/daily claim` | Claim today's reward directly | — |
| `/daily streak` | Show current streak and total claims | — |
| `/daily rewards` | Open the 30-day reward overview | — |
...
Reward types: `COINS`, `COSMETIC_CRATE`, `RANK_TEMP`, `ITEM`. Undefined intermediate days are interpolated from the next milestone for coin rewards.

---

## HmyAI

Paper plugin exposing `/ai` for in-game Claude tasks. The plugin starts a local callback server and invokes the configured Claude CLI command with server context.

### Commands

| Command | Description | Permission |
|---------|-------------|------------|
| `/ai <prompt>` | Ask Claude a question or request an in-game action | `hmy.ai` |
| `/ai reset` | Reset the player's conversation session | `hmy.ai` |
...
Keep `http-port` reachable only locally. Grant `hmy.ai` only to trusted users because the AI can call MCP tools such as command execution, player/world inspection, block lookup, and chat messages.
