...
public class SpleefManager {
    ...
    private final Map<UUID, SpleefGame> activeGames = new ConcurrentHashMap<>();
    private BukkitTask countdownTask;
    private int countdownTimer = -1;

    public SpleefManager(HmySpleef plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean leaveQueue(Player player) {
        ...
        if (isInGame(uuid)) {
            SpleefGame game = activeGames.get(uuid);
            if (game != null) {
    ...
    private void startCountdown() {
        ...
        countdownTask = plugin.getServer().getScheduler().runTaskTimer(plugin, () -> {
            ...
            if (countdownTimer <= 5 || countdownTimer == 10 || countdownTimer == 20) {
    ...
    private void tryStartGame() {
        ...
        SpleefArena.ArenaData arena = plugin.getSpleefArena().getFreeArena();
        if (arena == null) {
        ...
        SpleefGame game = new SpleefGame(plugin, players, arena);
        for (Player p : players) {
    ...
    public SpleefGame getGame(UUID uuid) {
    ...
    public void shutdown() {
        cancelCountdown();
        Set<SpleefGame> games = new HashSet<>(activeGames.values());
